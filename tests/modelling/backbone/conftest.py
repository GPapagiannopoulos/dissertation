"""Fixtures for the ported backbone: the numerical oracle and the vocabulary.

Every layer test here compares against a dump of the released JAX model, per layer.
The port is developed against motor_output/oracle_fp32.npz, where one float16 ULP
would otherwise hide a real bug; oracle_fp16.npz is the end-of-port fidelity check,
because float16 is the path real inference takes. Both artifacts are gitignored, so
the fixtures skip when they are absent.
"""
...
from thesis.modelling.backbone.tokenizer import TokenTable
...
def _open_oracle(path: Path, flag: str) -> NpzFile:
    ...
    if not path.exists():
        pytest.skip(
            ...
            f"scripts/tools/dump_motor_oracle.py --dtype {flag}'"
        )
    return np.load(path)
