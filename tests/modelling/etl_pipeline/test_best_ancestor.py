...
@pytest.mark.parametrize(
    "code, expected",
    [
        # 0. A token answers itself without climbing
        ("T", ("T", 0)),
...
def test_climbs_to_the_nearest_token(
    make_vocab: Callable, code: str, expected: tuple[str, int] | None
) -> None:
    """Asserts the search reports the token it reached and the layers it took."""
...
def test_prefers_the_nearer_token_over_the_more_informative_one(
    ...
    assert _best_ancestor("X", vocab, {}, {}) == ("NEAR", 1)
...
def test_prefers_the_most_informative_token_within_a_layer(
    ...
    assert _best_ancestor("X", vocab, {}, {}) == ("SHARP", 1)
...
def test_breaks_a_full_tie_on_the_code_itself(make_vocab: Callable) -> None:
    ...
    assert _best_ancestor("X", vocab, {}, {}) == ("A", 1)
...
def test_answers_from_the_memo_without_climbing(make_vocab: Callable) -> None:
    ...
    vocab = _vocab(make_vocab, CHAIN, {"T": -0.5})
    memo: dict[str, tuple[str, int] | None] = {"G": ("PLANTED", 9)}

    assert _best_ancestor("G", vocab, {}, memo) == ("PLANTED", 9)
