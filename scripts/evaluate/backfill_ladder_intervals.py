...
Run from the repo root with the modelling interpreter:
    ...
    .venv-modelling/bin/python scripts/evaluate/backfill_ladder_intervals.py \
        --runs motor_output/runs/ng-* --selection selection_test

`score_checkpoints.py` bands only AUPRC, so every other metric on a ladder carries a
point estimate and no interval. A table mixing the two reads as though some arms were
measured more precisely than others. This fills the gap from the banked
`*_predictions.npz`, so it needs no forward pass and no GPU.

Every metric is collected on ONE sequence of draws rather than one pass per metric,
which is five times cheaper and is also what makes the check below possible: the draw
order is `bootstrap_interval`'s exactly, so the AUPRC band recomputed here must equal
the one already in the file. It is asserted, and a mismatch means the two are not the
same construction and none of the backfilled bands can be trusted.
"""
...
def default_resamples() -> int:
    """The budget `score_checkpoints.py` banked with, so the bands stay comparable."""
    import importlib.util

    path = ROOT / "scripts" / "evaluate" / "score_checkpoints.py"
...
def all_intervals(
    ...
    *,
    ...
    """Every reported metric's interval, off one shared sequence of subject draws.
    ...
    for _ in range(resamples):
        ...
        for metric in REPORTED:
            ...
            # does -- a draw holding no positive has no AUPRC to contribute
...
def check_witness(row: dict, bands: dict[str, tuple[float, float]]) -> None:
    """Confirms the recomputed AUPRC band reproduces the one already banked.
