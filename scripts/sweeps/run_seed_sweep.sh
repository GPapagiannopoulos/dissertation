...
#     scripts/sweeps/run_seed_sweep.sh 1 2 3 4          # seeds to run
#     STEPS=15000 scripts/sweeps/run_seed_sweep.sh 1 2  # override the budget
#
...
for seed in "$@"; do
    ...
    if [ -e "${dest}" ]; then
        ...
        "${PYTHON}" "${ROOT}/scripts/train/train_motor_aki.py" \
            --dest "${dest}" --seed "${seed}" \
